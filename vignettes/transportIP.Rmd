...
1. The response and treatment columns for the target data are $\mathtt{NA}$, and
2. There is a binary variable indicating which observations are in the study data and the target data, with participation being coded as $\mathtt{1}$ or $\mathtt{TRUE}$.
...
We can now perform transportability analysis with the IOPW approach using the $\mathtt{transportIP}$ function. This function needs:

* A formula expressing the marginal structural model (MSM) to be fit, set in the $\mathtt{msmFormula}$ argument,
* A formula or a $\mathtt{glm}$ object expressing the propensity model i.e. a model of treatment assignment in terms of covariates, set in the $\mathtt{propensityScoreModel}$ argument. If a formula is provided, logistic regression is used by default. Custom propensity weights from other weighting methods can also be provided to the $\mathtt{customPropensity}$ argument instead; in this case, do not set $\mathtt{propensityScoreModel}$ because it is $\mathtt{NULL}$ by default and will be overridden.
* A formula or a $\mathtt{glm}$ object expressing the participation model i.e. a model of study participation in terms of effect modifiers, set in the $\mathtt{participationModel}$ argument. If a formula is provided, logistic regression is used by default. Custom participation weights from other weighting methods can also be provided to the $\mathtt{customParticipation}$ argument instead; in this case, do not set $\mathtt{participationModel}$ because it is $\mathtt{NULL}$ by default and will be overridden.
* The type of MSM to be fit, set in the $\mathtt{family}$ argument. This can be any of the families that are used in $\mathtt{glm}$ or one of $\mathtt{"coxph"}$ or $\mathtt{"survreg"}$. The latter options are for survival analysis and will use default options of these methods from the $\mathtt{survival}$ package.
* The study and target data, separate or merged, set in the $\mathtt{data}$ argument
* Whether a transportability analysis or generalizability analysis is done, set in the $\mathtt{\transport}$ argument (true for transportability, which is by default). Generalizability analysis weighs by inverse probability of study participation rather than odds.

These components are put together as follows. Here, $\mathtt{sysBloodPressure}$ is the response and $\mathtt{med1}$ is the treatment. Recall that sex, percent body fat and stress are covariates controlled for in the study, while other medication and stress are effect modifiers.
...
```
To show the results of the analysis, use the $\mathtt{summary}$ function like you would for $\mathtt{lm}$. This prints out covariate balance tables pre- and post-weighting for covariates between treatment groups (using propensity weights only) and effect modifiers (using participation weights only) between study and target data, as well as a summary output of the MSM model fit with the correct standard errors calculated by $\mathtt{sandwich::vcovBS}$. For the effect modifiers balance table, the weights used are inverse odds for study data and 1 for target data in a transportability analysis, and inverse probability for all observations in a generalizability analysis. Note that if custom participation weights are provided, the balance tables default to a transportability analysis since only the weights for observations in the study data are provided.

```{r IPWeighting-summary, echo = T}
summary(result)
```

The $\mathtt{transportIP}$ object produced by the $\mathtt{transportIP}$ function contains the model fitting objects for the propensity model, the participation model and the MSM. You can use methods like $\mathtt{coef}$ and $\mathtt{residuals}$ on these objects themselves. This is not implemented by the package because they are not as useful as implementing $\mathtt{summary}$.
...
Model coefficient plots showing confidence intervals of the effect estimates are provided by $\mathtt{plot}$ function with $\mathtt{type = "msm"}$. The standard errors are the correct ones calculated by $\mathtt{sandwich::vcovBS}$.

```{r IPWeighting-msm, echo = T}
plot(result, type = "msm")
```

Note that all plot outputs are $\mathtt{ggplot}$s, so if you have $\mathtt{ggplot2}$ installed, you can customize them further.